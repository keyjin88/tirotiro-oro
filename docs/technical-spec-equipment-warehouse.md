...
- Для дневного режима UI может принимать даты без времени, но серверная часть хранит нормализованный временной интервал.
...
- Система обязана повторно проверять доступность на серверной части при отправке формы.
...
- Удаление позиций оборудования и инвентарных единиц всегда выполняется как логическое удаление (soft delete).
...
- Логически удаленное оборудование не показывается в обычном каталоге и не доступно для новых бронирований.
...
- Java 25 LTS можно рассмотреть после подтверждения поддержки инфраструктурой и базовым Docker-образом.
- Spring Boot 4.1.x как актуальная стабильная линия на июнь 2026.
- Spring Framework 7.x, поставляемый через Spring Boot 4.1.x.
- Spring Security как обязательный модуль аутентификации, авторизации, CSRF-защиты и проверки ролей и прав.
- Spring Data JPA.
...
Такой подход оставляет основную бизнес-логику на серверной части, упрощает развертывание одним Docker-образом и не требует отдельной сборки клиентского приложения.

### База данных и миграции

- PostgreSQL 16+.
- changelog Liquibase в YAML или XML.
...
- `auth`: пользователи, роли, права, вход/выход, текущий пользователь.
...
- серверная часть возвращает HTML-фрагменты календарной сетки или строк таблицы.
...
В MVP предпочтительно не дублировать бизнес-логику между HTML и API: маршруты должны вызывать одни и те же сервисы приложения.
...
2. Получить и заблокировать строку `equipment_items` для выбранных позиций через `SELECT ... FOR UPDATE` или пессимистическую блокировку JPA.
...
Для UI с серверным рендерингом рекомендуется классическая сессионная аутентификация Spring Security:

- форма входа;
- защищенные cookies;
- включенная CSRF protection;
- logout через POST;
...
- frontend-шаблоны с серверным рендерингом;
- статические ресурсы;
- миграции Liquibase.
...
Рабочее развертывание предполагает внешнюю PostgreSQL:
...
- Для месячного календаря агрегировать данные на серверной части, а не отдавать полный список всех бронирований без фильтра.
...
- Резервное копирование PostgreSQL настраивается вне приложения.
...
- Нет отдельного жизненного цикла для выдачи и возврата оборудования.
